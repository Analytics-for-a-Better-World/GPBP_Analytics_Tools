...
def DistanceCalculation(network, hospitals, population, pop_subset, S1):
    ...
    for each_hospital in hospitals[['nearest_node', 'Longitude', 'Latitude']].values:
        ...
        pop_subset['euclidean_distance'] = haversine_vectorize(pop_subset['xcoord'], pop_subset['ycoord'],
                                                               pop_subset['hosp_lon'], pop_subset['hosp_lat'])
    ...
    df_matrix['shortest_path_length'] = network.shortest_path_lengths(df_matrix['nearest_node_pop'],
                                                                      df_matrix['nearest_node_hosp'])
    df_matrix = pd.merge(df_matrix, population[
        ['ID', 'nearest_node', 'pop_dist_road_estrada', 'household_count', 'xcoord', 'ycoord']],
                         right_on='nearest_node', left_on='nearest_node_pop')
    df_matrix = pd.merge(df_matrix, hospitals, right_on='nearest_node', left_on='nearest_node_hosp')

    df_matrix['euclidean_distance'] = haversine_vectorize(df_matrix['xcoord'], df_matrix['ycoord'],
                                                          df_matrix['Longitude'], df_matrix['Latitude'])
    df_matrix['total_network_distance'] = df_matrix['pop_dist_road_estrada'] + df_matrix['hosp_dist_road_estrada'] + \
                                          df_matrix['shortest_path_length']

    df_matrix['distance_corrected'] = df_matrix[
        ['pop_dist_road_estrada', 'euclidean_distance', 'total_network_distance']].swifter.apply(get_corrected_distance,
                                                                                                 axis=1)
